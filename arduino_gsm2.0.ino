...
void idea()
{
  gsm.println("ATD*121*46#;");
  delay(1000);
}
void bsnl()
{
  gsm.print("AT+CMGS=\"+91123\"\r");   //phone number with country code
  delay(50);
  gsm.print("PCNINFO NO"); //message to send
  delay(50);
  gsm.print((char)33);
  delay(1000);
  DELAY();
...
{ 
  ...
  while(gsm.available())
  {
    carrierinfo[i]=gsm.read();
    ++i;
  }
  if(carrierinfo[21]=='E')
  idea();
  else if(carrierinfo[21]=='B');
  //bsnl();
  else if(carrierinfo[21]=='A');
  //airtel;
}
...
void message()
{
  ...
  gsm.print("AT+CMGS=\"+919077700363\"\r");   //phone number with country code
  delay(50);
